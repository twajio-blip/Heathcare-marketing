...
export function renderInnovation() {
    ...
    <div
      ...
      <a href="${item.linkUrl}"
        ...
        <span
          class="px-large py-small rounded-full bg-linear-to-r from-${item.color}-700 to-${item.color}-900 border border-${item.color}-400/30 text-white font-bold text-size-accent tracking-wide flex items-center gap-small shadow-lg shadow-${item.color}-950/50 hover:scale-105 hover:border-${item.color}-400/60 transition-all duration-300">
      ...
      <div class="relative z-0 flex flex-col h-full">
        ...
        <h3 class="text-size-body text-skin-accent-3 mb-small tracking-tight">${item.title}</h3>
        <p
          class="text-size-accent text-skin-accent-3/70 leading-relaxed group-hover:text-skin-accent-3/50 transition-colors duration-500 grow">
          ${item.description}
        </p>
      </div>
    </div>
